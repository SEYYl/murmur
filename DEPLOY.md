...
| Render (Web Service) | 750h/月 + 冷休眠 | $0（存设置到环境变量，无需 Disk）|
| Cloudflare R2 | 10GB 存储 + 1000万次读 | $0 |

**总额：$0/月（配置通过环境变量持久化，无需 Render Disk）**
...
#### 3.5 持久化配置：不用 Disk 的方案（推荐）

前面说了 Render 的 Disk 要 $0.25/月。但代码现在支持**环境变量配置 R2**，完全跳过 Disk：

在 Render 服务 → Environment 里添加以下环境变量（不用挂 Disk，部署也不丢）：

| 环境变量 | 值 | 对应后台设置 |
|---------|-----|------------|
| `MURMUR_STORAGE_BACKEND` | `s3` | 存储后端 |
| `MURMUR_STORAGE_PROVIDER` | `cloudflare` | 存储服务提供商 |
| `MURMUR_S3_REGION` | 你的 Cloudflare Account ID | Region |
| `MURMUR_S3_BUCKET` | 你的桶名（如 `murmur-media`） | Bucket |
| `MURMUR_S3_ACCESS_KEY` | 令牌的「访问密钥 ID」 | Access Key |
| `MURMUR_S3_SECRET_KEY` | 令牌的「机密访问密钥」 | Secret Key |

加好之后点 **「Save Changes」**，Render 会自动重启服务。之后**不管部署多少次，配置都不会丢**，而且彻底省掉 Disk 费用。

> 环境变量设置后，后台的「存储设置」页仍然可以修改——**环境变量优先级更高**，会覆盖数据库里的配置。如果要切回后台设置，删除对应的环境变量即可。
...
#### ❌ 每次部署后设置就丢了，R2 配置要重新填

**原因**：Render 免费版没有持久化磁盘（Disk 要 $0.25/月），容器重建后 SQLite 数据库重置。

**解决**：用环境变量配置 R2（详见上方「3.5 持久化配置」），配置存在 Render 的环境变量里，部署不丢，还省了 Disk 费用。
